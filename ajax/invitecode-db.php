...
if (isset($_POST['draw'])) {
    ...
    $searchQuery = " ";
    if ($searchValue != '') {
        $searchQuery = " and (`$table_name`.`invite_id` like '%" . $searchValue . "%' ) ";
    }
    ...
    ## Fetch records
    $empQuery = "select `$table_name`.* from `$table_name` WHERE 1 " . $searchQuery. " order by `" . $columnName . "` " . $columnSortOrder . " limit " . $row . "," . $rowperpage;
